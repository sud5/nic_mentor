...
//       echo $fields . $sql;die;
    ...
    public static function mentor_school_database_parameters() {
        return new external_function_parameters(
                array(
                    'page' => new external_value(PARAM_INT, 'page'),
                    'name' => new external_value(PARAM_RAW, 'name')
    ...
    public static function mentor_school_database($page, $name) {
        global $DB, $PAGE;
        $params = self::validate_parameters(self::mentor_school_database_parameters(), array('page' => $page, 'name' => $name));
        $out = '';
        $context = \context_system::instance();
        $PAGE->set_context($context);
        $perpage = 20;
        ...
        $renderer = $PAGE->get_renderer('local_mentor');
        $out = "";
        $out .= $renderer->school_database($availablementors, count($allavailablementors), $page, $perpage, $attrparams);
        $html = array();
        $html['html'] = $out;
        return $html;
    }
     ...
     * @return external_description
     */
    public static function mentor_school_database_returns() {
        return $data = new external_single_structure([
            'html' => new external_value(PARAM_RAW, 'html')
        ]);
    }
